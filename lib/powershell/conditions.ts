...
export class PropertyCondition extends Condition {
  constructor(property: Property, value: PSObject) {
    property = property.toLowerCase().endsWith('property')
      ? (property.slice(0, property.length - 8) as Property)
      : property;
    ...
    if (Object.values(OrientationTypeProperty).includes(property as OrientationTypeProperty)) {
      try {
        ...
        if (
          value instanceof PSAutomationHeadingLevel &&
          value.originalValue.toLowerCase() === AutomationHeadingLevel.NONE
        ) {
          value = new PSOrientationType(OrientationType.NONE);
        } else if (
          !(value instanceof PSInt32) ||
  ...
  constructor(...conditions: Condition[]) {
    if (!conditions.every((arg) => arg instanceof Condition)) {
      throw new errors.InvalidArgumentError(
        `AndCondition expects Conditions as args but received ${(conditions.find((x) => !(x instanceof Condition)) as unknown)?.constructor.name}.`,
      );
    }

    if (conditions.length < 2) {
      throw new errors.InvalidArgumentError(
        `AndCondition must have at least 2 conditions, but received ${conditions.length}.`,
      );
    }
...
export class OrCondition extends Condition {
  constructor(...conditions: Condition[]) {
    if (!conditions.every((arg) => arg instanceof Condition)) {
      throw new errors.InvalidArgumentError(
        `OrCondition expects Conditions as args but received ${(conditions.find((x) => !(x instanceof Condition)) as unknown)?.constructor.name}.`,
      );
    }

    if (conditions.length < 2) {
      throw new errors.InvalidArgumentError(
        `OrCondition must have at least 2 conditions, but received ${conditions.length}.`,
      );
    }
...
export class NotCondition extends Condition {
  constructor(condition: Condition) {
    if (!(condition instanceof Condition)) {
      throw new errors.InvalidArgumentError(
        `AndCondition expects Conditions as args but received ${(condition as unknown)?.constructor.name}.`,
      );
    }
...
function assertPSObjectType(obj: PSObject, type: new (...args: any[]) => PSObject) {
  if (!(obj instanceof type)) {
    throw new errors.InvalidArgumentError(
      `Property expected type ${type.name} but got ${(obj as object)?.constructor.name}.`,
    );
  }
}
