...
export function convertStringToCondition(selector: string): Condition {
  ...
  processedSelector = processedSelector.replaceAll(
    RECT_PARAMETER_REGEX_C2,
    (_, x: string, y: string, width: string, height: string) => {
      const replacementChar = String.fromCharCode(MAGIC_PLACEHOLDER_UNICODE_BEGIN + processedItems.length);
  ...
  processedSelector = processedSelector.replaceAll(
    CULTURE_INFO_PARAMETER_REGEX_C2,
    (_, name: string, useUserOverride: string) => {
      const replacementChar = String.fromCharCode(MAGIC_PLACEHOLDER_UNICODE_BEGIN + processedItems.length);
  ...
  processedSelector = processedSelector.replaceAll(
    CULTURE_INFO_PARAMETER_REGEX_C4,
    (_, culture: string, useUserOverride: string) => {
      const replacementChar = String.fromCharCode(MAGIC_PLACEHOLDER_UNICODE_BEGIN + processedItems.length);
  ...
  processedSelector = processedSelector.replaceAll(CONTENT_VIEW_CONDITION_REGEX, () => {
    ...
    processedItems.push(
      new NotCondition(
        new OrCondition(
          new PropertyCondition(Property.IS_CONTROL_ELEMENT, new PSBoolean(false)),
          new PropertyCondition(Property.IS_CONTENT_ELEMENT, new PSBoolean(false)),
        ),
      ),
    );
    return replacementChar;
  ...
  processedSelector = processedSelector.replaceAll(
    ...
    (_, property: string, processedItem: string) => {
      ...
      if (
        propertyValue instanceof PSControlType &&
        Object.values(ExtraControlType).includes(propertyValue.toString().toLocaleLowerCase() as ExtraControlType)
      ) {
        processedItems.push(
          new PropertyCondition(Property.LOCALIZED_CONTROL_TYPE, new PSString(propertyValue.toString())),
        );
      } else {
  ...
  while (ANY_LOGIC_CONDITION_REGEX.test(processedSelector)) {
    processedSelector = processedSelector.replaceAll(AND_CONDITION_REGEX, (_, content: string) => {
      ...
      const args = content
        .split(',')
        .map((x) => x.trim())
        .map((x) => processedItems[x.charCodeAt(0) - MAGIC_PLACEHOLDER_UNICODE_BEGIN]);
