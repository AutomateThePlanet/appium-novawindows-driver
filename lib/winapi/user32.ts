...
const GetWindowTextA = user32.func(
  /* c */ `int __stdcall GetWindowTextA(HWND hWnd, LPSTR lpString, int nMaxCount)`,
) as (hWnd: HWND, lpString: LPSTR, nMaxCount: number) => number;
  ...
  if (easingFunction) {
    ...
    for (let i = 1; i < iterations; i++) {
      setTimeout(() => {
        ...
        const events = makeMouseMoveEvents({
          x: interpolatedX,
          y: interpolatedY,
          wheel: false,
          screenResolutionAndRefreshRate,
        });
        const returnCode = SendInput(events.length, events, sizeof(INPUT));
...
// TODO: Remove eslint-disable-next-line when used
// eslint-disable-next-line @typescript-eslint/no-unused-vars
let getResolutionScalingFactor = (): number => {
...
let getScreenResolutionAndRefreshRate = (): [number, number, number] => {
...
export function getWindowAllHandlesForProcessIds(processIds: number[]): number[] {
  ...
  EnumWindows((hWnd) => {
    ...
    if (pid && processIds.includes(pid) && IsWindowVisible(hWnd)) {
      ...
      const windowTitle = buffer.toString('utf8').split('\0')[0];
