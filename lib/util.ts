...
export async function getBundledFfmpegPath(driver: NovaWindowsDriver): Promise<string | null> {
  ...
  if (ffmpegExecutablePath) {
    ...
    if (!exists) {
      throw new errors.InvalidArgumentError(`ffmpeg executable not found at: ${ffmpegExecutablePath}`);
    }
  ...
  try {
    ...
    if (!res.ok) {
      throw new errors.UnknownError(`Failed to fetch ffmpeg release: ${res.status}`);
    }
    ...
    const asset = release.assets?.find((a) => a.name.endsWith('full_build.zip'));

    if (!asset?.browser_download_url) {
      throw new errors.UnknownError('No ffmpeg full_build.zip asset found in latest release');
    }
    ...
    const downloadedZip = path.join(tmpRoot, path.basename(asset.browser_download_url));

    driver.log.info(`ffmpeg: verifying downloaded archive`);

    if (!(await fs.exists(downloadedZip))) {
      throw new errors.UnknownError(`Downloaded ffmpeg zip not found at ${downloadedZip}`);
    }
    ...
    const found = await fs.walkDir(
      ...
      (itemPath: string, isDirectory: boolean) =>
        !isDirectory && path.basename(itemPath).toLowerCase() === 'ffmpeg.exe',
    );
...
export async function downloadFile(url: string, destPath: string, timeout = 30000): Promise<void> {
...
const SupportedEasingFunctions = Object.freeze(['linear', 'ease', 'ease-in', 'ease-out', 'ease-in-out']);

export function assertSupportedEasingFunction(value: string) {
  const cubicBezierRegex =
    /^cubic-bezier\(\s*(0|1|0?\.\d+|\d+(\.\d+)?)\s*,\s*(-?0|-?1|-?0?\.\d+|-?\d+(\.\d+)?)\s*,\s*(0|1|0?\.\d+|\d+(\.\d+)?)\s*,\s*(-?0|-?1|-?0?\.\d+|-?\d+(\.\d+)?)\s*\)$/;
  if (!SupportedEasingFunctions.includes(value) && !cubicBezierRegex.test(value)) {
    throw new errors.InvalidArgumentError(
      `Unsupported or invalid easing function '${value}' in appium:smoothPointerMove capability.` +
