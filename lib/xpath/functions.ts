...
import type {FunctionName, ExprNode} from 'xpath-analyzer';
import {
  BOOLEAN,
  CONCAT,
  CONTAINS,
...
export async function handleFunctionCall<T>(
  name: FunctionName,
  ...
  switch (name) {
    ...
    case CONCAT: {
      ...
      if (resultArrays.some((resultArray) => resultArray.length > 1)) {
        throw new errors.InvalidArgumentError(
          FUNCTION_ARGUMENT_ERROR.format(name, 'each argument to have either one or zero elements'),
        );
      }
    ...
    case CONTAINS: {
      ...
      const [firstArgResult, secondArgResult] = await processArgs<string | boolean | AutomationElement>(
        args[0],
        args[1],
      );
      ...
      if (typeof lhs === 'boolean') {
        throw new errors.InvalidArgumentError(
          FUNCTION_ARGUMENT_ERROR.format(name, 'the first argument to be string, number or element'),
        );
      }

      if (typeof rhs === 'boolean') {
        throw new errors.InvalidArgumentError(
          FUNCTION_ARGUMENT_ERROR.format(name, 'the second argument to be string, number or element'),
        );
      }
    ...
    case FLOOR: {
      ...
      if (typeof num !== 'number') {
        throw new errors.InvalidArgumentError(
          FUNCTION_ARGUMENT_ERROR.format(name, 'the first argument to be a number'),
        );
      }
    ...
    case ID: {
      ...
      const conditions = ids.map(
        (id) => new PropertyCondition(Property.RUNTIME_ID, new PSInt32Array(id.split('.').map(Number))),
      );
      const results: string[] = [];
      for (const condition of conditions) {
        results.push(
          await sendPowerShellCommand(
            AutomationElement.rootElement.findFirst(TreeScope.SUBTREE, condition).buildCommand(),
          ),
        );
      }
    ...
    case NORMALIZE_SPACE: {
      ...
      throw new errors.InvalidArgumentError(
        FUNCTION_ARGUMENT_ERROR.format(name, 'the first argument to be string or element'),
      );
    }
    case STRING_LENGTH: {
      if (args.length > 1) {
      ...
      if (argResult && argResult.length > 1) {
        throw new errors.InvalidArgumentError(
          FUNCTION_ARGUMENT_ERROR.format(name, 'the first argument to have either one or zero elements'),
        );
      }
    ...
    case TRANSLATE: {
      ...
      const [firstArgResult, secondArgResult, thirdArgResult] = await processArgs<string | AutomationElement>(
        args[0],
        args[1],
        args[2],
      );
      ...
      if (![str, from, to].every((x) => typeof x === 'string' || x instanceof AutomationElement)) {
        throw new errors.InvalidArgumentError(
          FUNCTION_ARGUMENT_ERROR.format(name, 'each argument to be string or element'),
        );
      }
    ...
    case STRING: {
      ...
      return name === STRING
        ? [...(convertProcessedExprNodesToStrings(firstArg ?? [context]) as T[])]
        : [...(convertProcessedExprNodesToNumbers(firstArg ?? [context]) as T[])];
    }
...
function convertProcessedExprNodesToNumbers<T>(...arrayOfProcessedExprNodes: T[]): number[] {
  ...
  return arrayOfStrings.map((str) =>
    /^\s*(?<![\d.+-])[+-]?(?:\d*[.])?\d+(?![\d.])\s*$/.test(str) ? NaN : Number(str),
  );
}
