...
import type {ExprNode, LocationNode, StepNode, NodeTestNode} from 'xpath-analyzer';
...
import type {Condition} from '../powershell';
import {
  Property,
  PSControlType,
  FalseCondition,
...
export async function xpathToElIdOrIds(
  ...
  sendPowerShellCommand: (command: string) => Promise<string>,
  ...
  const foundElements = await processExprNode<FoundAutomationElement>(
    parsedXPath,
    context ? new FoundAutomationElement(context) : AutomationElement.automationRoot,
    sendPowerShellCommand,
  );
  const els = foundElements
    .filter((el) => el instanceof FoundAutomationElement)
    .map((el) => ({[W3C_ELEMENT_KEY]: el.runtimeId}));
...
export async function processExprNode<T>(
  exprNode: ExprNode,
  context: AutomationElement,
  sendPowerShellCommand: (command: string) => Promise<string>,
): Promise<T[]> {
  switch (exprNode.type) {
    ...
    case UNION:
      return [
        ...(await processExprNode<T>(exprNode.lhs, context, sendPowerShellCommand)),
        ...(await processExprNode<T>(exprNode.rhs, context, sendPowerShellCommand)),
      ];
    case FUNCTION_CALL:
...
async function handleLocationNode(
  location: LocationNode,
  context: AutomationElement,
  sendPowerShellCommand: (command: string) => Promise<string>,
): Promise<AutomationElement[] | string[]> {
  ...
  if (context instanceof AutomationElementGroup && context.groups.length > 1) {
    throw new errors.InvalidArgumentError(
      `handleLocationNode expects single context, but received ${context.groups.length} contexts.`,
    );
  }
...
export async function processExprNodeAsPredicate(
  exprNode: ExprNode,
  context: AutomationElement,
  positions: Set<number>,
  sendPowerShellCommand: (command: string) => Promise<string>,
  relativeExprNodes?: ExprNode[],
): Promise<[Condition, ExprNode[]?]> {
  relativeExprNodes ??= [];
  switch (exprNode.type) {
    case NUMBER:
    ...
    case OR:
      return [
        new OrCondition(
          (
            await processExprNodeAsPredicate(exprNode.lhs, context, positions, sendPowerShellCommand, relativeExprNodes)
          )[0],
          (
            await processExprNodeAsPredicate(exprNode.rhs, context, positions, sendPowerShellCommand, relativeExprNodes)
          )[0],
        ),
        relativeExprNodes,
      ];
      ...
      if ((exprNode.lhs.type === RELATIVE_LOCATION_PATH) !== (exprNode.rhs.type === RELATIVE_LOCATION_PATH)) {
        if (
          ...
          if (propertyName === Property.RUNTIME_ID) {
            return [
              new PropertyCondition(propertyName, new PSInt32Array(String(value).split('.').map(Number))),
              relativeExprNodes,
            ];
          }
        ...
        (exprNode.lhs.type === FUNCTION_CALL && exprNode.lhs.name === POSITION) !==
        (exprNode.rhs.type === FUNCTION_CALL && exprNode.rhs.name === POSITION)
      ) {
...
async function executeStep(
  ...
  const condition =
    predicateConditions.length > 0
      ? new AndCondition(convertNodeTestToCondition(step.test), ...predicateConditions)
      : convertNodeTestToCondition(step.test);

  let find: AutomationElement;
  switch (step.axis) {
    case ANCESTOR:
      find = step[OptimizeLastStep]
        ? context.findFirst(TreeScope.ANCESTORS, condition)
        : context.findAll(TreeScope.ANCESTORS, condition);
      break;
    case ANCESTOR_OR_SELF:
      find = step[OptimizeLastStep]
        ? context.findFirst(TreeScope.ANCESTORS_OR_SELF, condition)
        : context.findAll(TreeScope.ANCESTORS_OR_SELF, condition);
      break;
    case CHILD:
      find = step[OptimizeLastStep]
        ? context.findFirst(TreeScope.CHILDREN, condition)
        : context.findAll(TreeScope.CHILDREN, condition);
      break;
    case DESCENDANT:
      find = step[OptimizeLastStep]
        ? context.findFirst(TreeScope.DESCENDANTS, condition)
        : context.findAll(TreeScope.DESCENDANTS, condition);
      break;
    case DESCENDANT_OR_SELF:
      find = step[OptimizeLastStep]
        ? context.findFirst(TreeScope.SUBTREE, condition)
        : context.findAll(TreeScope.SUBTREE, condition);
      break;
    case FOLLOWING:
      find = step[OptimizeLastStep]
        ? context.findFirst(TreeScope.FOLLOWING, condition)
        : context.findAll(TreeScope.FOLLOWING, condition);
      break;
    case FOLLOWING_SIBLING:
      find = step[OptimizeLastStep]
        ? context.findFirst(TreeScope.FOLLOWING_SIBLING, condition)
        : context.findAll(TreeScope.FOLLOWING_SIBLING, condition);
      break;
    case NAMESPACE:
      return new AutomationElementGroup(/* empty */);
    case PARENT:
      find = step[OptimizeLastStep]
        ? context.findFirst(TreeScope.PARENT, condition)
        : context.findAll(TreeScope.PARENT, condition);
      break;
    case PRECEDING:
      find = step[OptimizeLastStep]
        ? context.findFirst(TreeScope.PRECEDING, condition)
        : context.findAll(TreeScope.PRECEDING, condition);
      break;
    case PRECEDING_SIBLING:
      find = step[OptimizeLastStep]
        ? context.findFirst(TreeScope.PRECEDING_SIBLING, condition)
        : context.findAll(TreeScope.PRECEDING_SIBLING, condition);
      break;
    case SELF:
      find = step[OptimizeLastStep]
        ? context.findFirst(TreeScope.ELEMENT, condition)
        : context.findAll(TreeScope.ELEMENT, condition);
      break;
    default:
  ...
  if (positionsArray.length === 0) {
    ...
    return new AutomationElementGroup(
      ...positionsArray
    ...
    case NODE_NAME_TEST:
      ...
      if (nodeTest.name.toLowerCase() === 'appbar' || nodeTest.name.toLowerCase() === 'semanticzoom') {
        ...
        return new PropertyCondition(
          Property.LOCALIZED_CONTROL_TYPE,
          new PSString(new PSControlType(nodeTest.name).toString()),
        );
      }
...
async function convertAttributeNodeTestToStringArray(
  nodeTest: NodeTestNode,
  context: AutomationElement,
  sendPowerShellCommand: (command: string) => Promise<string>,
): Promise<string[]> {
  ...
  const els = result
    ...
    .map((id) => id.trim())
    .filter(Boolean)
    .map((id) => new FoundAutomationElement(id.trim()));
    ...
    case NODE_NAME_TEST:
      if (extraProperties.includes(nodeTest.name.toLowerCase())) {
        ...
        for (const el of els) {
          ...
          results.push(
            JSON.parse(rectJson.replaceAll(/(?:infinity)/gi, (0x7fffffff).toString()))[nodeTest.name.toLowerCase()],
          );
        }
...
function optimizeDoubleSlash(steps: StepNode[]): void {
  for (let i = 0; i < steps.length - 1; i++) {
    ...
    if (
      steps[i].axis === DESCENDANT_OR_SELF &&
      steps[i].test.type === NODE_TYPE_TEST &&
      ...
      if (
        steps[i].predicates.some(
          (predicate) => predicate.type === FUNCTION_CALL && (predicate.name === LAST || predicate.name === POSITION),
        )
      ) {
