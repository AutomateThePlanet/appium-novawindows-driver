...
export async function handleKeyActionSequence(
...
export async function handlePointerActionSequence(
  this: NovaWindowsDriver,
  ...
  switch (actionSequence.parameters?.pointerType) {
    ...
    case 'pen':
      throw new errors.NotImplementedError(
        `Pointer type ${actionSequence.parameters?.pointerType} not implemented yet.`,
      );
    case 'mouse':
    default:
...
export async function handleMousePointerActionSequence(
  this: NovaWindowsDriver,
  actionSequence: PointerActionSequence,
): Promise<void> {
  const actions = actionSequence.actions;
...
export async function handleWheelActionSequence(
  this: NovaWindowsDriver,
  actionSequence: WheelActionSequence,
): Promise<void> {
  const actions = actionSequence.actions;
  for (const action of actions) {
    switch (action.type) {
...
export async function handleNullActionSequence(
  this: NovaWindowsDriver,
  actionSequence: NullActionSequence,
): Promise<void> {
  const actions = actionSequence.actions;
  for (const action of actions) {
    if (action.duration) {
...
export async function handleMouseMoveAction(
  this: NovaWindowsDriver,
  action: PointerMoveAction | ScrollAction,
): Promise<void> {
  ...
  switch (action.origin) {
    case 'pointer':
      ...
      break;
    case 'viewport': {
      const rootRectJson = await this.sendPowerShellCommand(
        AutomationElement.automationRoot.buildGetElementRectCommand(),
      );
    ...
    default:
      if (action.origin?.[W3C_ELEMENT_KEY]) {
        ...
        await mouseMoveAbsolute(
          action.x === 0 ? rect.x + rect.width / 2 : action.x,
          action.y === 0 ? rect.y + rect.height / 2 : action.y,
          action.duration,
          easingFunction,
        );
        break;
