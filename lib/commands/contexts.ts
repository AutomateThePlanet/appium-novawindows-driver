...
export async function setContext(this: NovaWindowsDriver, name?: string | null): Promise<void> {
  ...
  const debuggerAddress = (page?.webSocketDebuggerUrl ?? '').replace('ws://', '').split('/')[0];
...
export async function getContexts(this: NovaWindowsDriver): Promise<string[]> {
  const webViewDetails = await this.getWebViewDetails();
  return [NATIVE_APP, ...(webViewDetails.pages?.map((page) => `${WEBVIEW_BASE}${page.id}`) ?? [])];
}
...
export async function getWebViewDetails(this: NovaWindowsDriver, waitForWebviewMs?: number): Promise<WebViewDetails> {
  if (!this.caps.webviewEnabled) {
    throw new errors.InvalidArgumentError(
      'WebView support is not enabled. Please set the "enableWebView" capability to true and try again.',
    );
  }
  ...
  if (
    ...
    throw new errors.InvalidArgumentError(
      `Capability "webviewDevtoolsPort" must be set when using "none", "root", or "appTopLevelWindow" with "enableWebView"`,
    );
  }
  ...
  const info = await (
    cdpRequest.call(this, {host, port, endpoint: '/json/version', timeout: 10000}) as Promise<CDPVersionResponse>
  ).catch(() => undefined);
  const pages = await (
    cdpRequest.call(this, {host, port, endpoint: '/json/list', timeout: 10000}) as Promise<CDPListResponse>
  ).catch(() => undefined);
...
async function getDriverExecutable(
  ...
  browserVersion: `${number}.${number}.${number}.${number}`,
): Promise<string> {
  ...
  const executablePath =
    browserType === 'Edge' ? this.caps.edgedriverExecutablePath : this.caps.chromedriverExecutablePath;
  ...
  try {
    ...
    if (!driverPath) {
      throw new errors.UnknownError(
        `The archive was unzipped properly, but did not find any ${driverType} executable.`,
      );
    }
