...
export async function setValue(this: NovaWindowsDriver, value: string | string[], elementId: string): Promise<void> {
  ...
  const sendKeysAndResetArray = async () => {
    await this.sendPowerShellCommand(
      /* ps1 */ `[Windows.Forms.SendKeys]::SendWait(${new PSString(keysToSend.join(''))})`,
    );
    keysToSend = [];
  };
...
export async function elementDisplayed(this: NovaWindowsDriver, elementId: string): Promise<boolean> {
  const result = await this.sendPowerShellCommand(
    new FoundAutomationElement(elementId).buildGetPropertyCommand(Property.IS_OFFSCREEN),
  );
...
export async function elementEnabled(this: NovaWindowsDriver, elementId: string): Promise<boolean> {
  const result = await this.sendPowerShellCommand(
    new FoundAutomationElement(elementId).buildGetPropertyCommand(Property.IS_ENABLED),
  );
...
export async function click(this: NovaWindowsDriver, elementId: string): Promise<void> {
  ...
  try {
    const focusableElementId = await this.sendPowerShellCommand(
  ...
  try {
    const clickablePointJson = await this.sendPowerShellCommand(
      element.buildGetPropertyCommand(Property.CLICKABLE_POINT),
    );
...
export async function getElementScreenshot(this: NovaWindowsDriver, elementId: string): Promise<string> {
  ...
  return await this.sendPowerShellCommand(GET_ELEMENT_SCREENSHOT_COMMAND.format(new FoundAutomationElement(elementId)));
}
