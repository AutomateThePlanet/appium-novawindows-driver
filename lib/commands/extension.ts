...
import {keyDown, keyUp, mouseDown, mouseMoveAbsolute, mouseScroll, mouseUp, sendKeyboardEvents} from '../winapi/user32';
...
const AUTOMATION_ELEMENT_MODE = $
/* ps1 */ `$cacheRequest.Pop(); $cacheRequest.AutomationElementMode = ${0}; $cacheRequest.Push()`;

const SET_PLAINTEXT_CLIPBOARD_FROM_BASE64 = $
/* ps1 */ `Set-Clipboard -Value ([System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String(${0})))`;
const GET_PLAINTEXT_CLIPBOARD_BASE64 = /* ps1 */ `[Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes((Get-Clipboard)))`;

const SET_IMAGE_CLIPBOARD_FROM_BASE64 = $
/* ps1 */ `$b = [Convert]::FromBase64String(${0}); $s = New-Object IO.MemoryStream; $s.Write($b, 0, $b.Length); $s.Position = 0; $i = [System.Windows.Media.Imaging.BitmapFrame]::Create($s); [Windows.Clipboard]::SetImage($i); $s.Close()`;
  ...
  if (script === 'mobile:getContexts') {
    if (!this.caps.webviewEnabled) {
      throw new errors.InvalidArgumentError(
        'WebView support is not enabled. To use this command, enable WebView support by setting the "enableWebView" capability to true.',
      );
    }
...
const TREE_SCOPE_REGEX = new PropertyRegexMatcher(
  'System.Windows.Automation.TreeScope',
  ...Object.values(TreeScope),
).toRegex('i');
const AUTOMATION_ELEMENT_MODE_REGEX = new PropertyRegexMatcher(
  'System.Windows.Automation.AutomationElementMode',
  ...Object.values(AutomationElementMode),
).toRegex('i');
...
export async function pushCacheRequest(this: NovaWindowsDriver, cacheRequest: CacheRequest): Promise<void> {
  ...
  if (cacheRequest.treeScope) {
    ...
    if (!treeScope || (Number(cacheRequest.treeScope) < 1 && Number(cacheRequest.treeScope) > 16)) {
      throw new errors.InvalidArgumentError(
        `Invalid value '${cacheRequest.treeScope}' passed to TreeScope for cache request.`,
      );
    }

    await this.sendPowerShellCommand(
      TREE_SCOPE_COMMAND.format(
        isNaN(Number(cacheRequest.treeScope))
          ? /* ps1 */ `[TreeScope]::${cacheRequest.treeScope}`
  ...
  if (cacheRequest.automationElementMode) {
    ...
    if (
      !treeScope ||
      (Number(cacheRequest.automationElementMode) < 0 && Number(cacheRequest.automationElementMode) > 1)
    ) {
      throw new errors.InvalidArgumentError(
        `Invalid value '${cacheRequest.automationElementMode}' passed to AutomationElementMode for cache request.`,
      );
    }
...
export async function patternIsMultiple(this: NovaWindowsDriver, element: Element): Promise<boolean> {
  const result = await this.sendPowerShellCommand(
    new FoundAutomationElement(element[W3C_ELEMENT_KEY]).buildIsMultipleSelectCommand(),
  );
...
export async function patternGetSelectedItem(this: NovaWindowsDriver, element: Element): Promise<Element> {
  const result = await this.sendPowerShellCommand(
    new FoundAutomationElement(element[W3C_ELEMENT_KEY]).buildGetSelectionCommand(),
  );
...
export async function patternRemoveFromSelection(this: NovaWindowsDriver, element: Element): Promise<void> {
  await this.sendPowerShellCommand(
    new FoundAutomationElement(element[W3C_ELEMENT_KEY]).buildRemoveFromSelectionCommand(),
  );
}
...
export async function patternSetValue(this: NovaWindowsDriver, element: Element, value: string): Promise<void> {
  try {
    ...
    await this.sendPowerShellCommand(
      new FoundAutomationElement(element[W3C_ELEMENT_KEY]).buildSetRangeValueCommand(value),
    );
  }
}
...
export async function executeClick(
  ...
  if (elementId) {
    if (await this.sendPowerShellCommand(/* ps1 */ `$null -eq ${new FoundAutomationElement(elementId).toString()}`)) {
      ...
      const elId = await this.sendPowerShellCommand(
    ...
    const rectJson = await this.sendPowerShellCommand(
      new FoundAutomationElement(elementId).buildGetElementRectCommand(),
    );
    const rect = JSON.parse(rectJson.replaceAll(/(?:infinity)/gi, (0x7fffffff).toString())) as Rect;
    pos = [rect.x + (x ?? Math.trunc(rect.width / 2)), rect.y + (y ?? Math.trunc(rect.height / 2))];
  } else {
...
export async function executeHover(
  ...
  if (startElementId) {
    if (
      await this.sendPowerShellCommand(/* ps1 */ `$null -eq ${new FoundAutomationElement(startElementId).toString()}`)
    ) {
      const condition = new PropertyCondition(
        Property.RUNTIME_ID,
        new PSInt32Array(startElementId.split('.').map(Number)),
      );
      const elId = await this.sendPowerShellCommand(
        AutomationElement.automationRoot.findFirst(TreeScope.SUBTREE, condition).buildCommand(),
      );
    ...
    const rectJson = await this.sendPowerShellCommand(
      new FoundAutomationElement(startElementId).buildGetElementRectCommand(),
    );
    const rect = JSON.parse(rectJson.replaceAll(/(?:infinity)/gi, (0x7fffffff).toString())) as Rect;
    startPos = [rect.x + (startX ?? rect.width / 2), rect.y + (startY ?? rect.height / 2)];
  } else {
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
  ...
  if (endElementId) {
    if (
      await this.sendPowerShellCommand(/* ps1 */ `$null -eq ${new FoundAutomationElement(endElementId).toString()}`)
    ) {
      const condition = new PropertyCondition(
        Property.RUNTIME_ID,
        new PSInt32Array(endElementId.split('.').map(Number)),
      );
      const elId = await this.sendPowerShellCommand(
        AutomationElement.automationRoot.findFirst(TreeScope.SUBTREE, condition).buildCommand(),
      );
    ...
    const rectJson = await this.sendPowerShellCommand(
      new FoundAutomationElement(endElementId).buildGetElementRectCommand(),
    );
    const rect = JSON.parse(rectJson.replaceAll(/(?:infinity)/gi, (0x7fffffff).toString())) as Rect;
    endPos = [rect.x + (endX ?? rect.width / 2), rect.y + (endY ?? rect.height / 2)];
  } else {
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    endPos = [endX!, endY!];
  }
