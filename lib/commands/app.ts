...
export async function maximizeWindow(this: NovaWindowsDriver): Promise<void> {
  const automationRoot = new FoundAutomationElement(
    AutomationElement.automationRoot.buildGetPropertyCommand(Property.RUNTIME_ID),
  );
  try {
...
export async function getWindowHandle(this: NovaWindowsDriver): Promise<string> {
  const nativeWindowHandle = await this.sendPowerShellCommand(
    AutomationElement.automationRoot.buildGetPropertyCommand(Property.NATIVE_WINDOW_HANDLE),
  );
...
export async function getWindowHandles(this: NovaWindowsDriver): Promise<string[]> {
  const result = await this.sendPowerShellCommand(
    AutomationElement.rootElement.findAll(TreeScope.CHILDREN, new TrueCondition()).buildCommand(),
  );
  const elIds = result
    .split('\n')
    .map((x) => x.trim())
  ...
  for (const elId of elIds) {
    const nativeWindowHandle = await this.sendPowerShellCommand(
      new FoundAutomationElement(elId).buildGetPropertyCommand(Property.NATIVE_WINDOW_HANDLE),
    );
...
export async function setWindow(this: NovaWindowsDriver, nameOrHandle: string): Promise<void> {
  ...
  for (let i = 1; i <= 20; i++) {
    // TODO: make a setting for the number of retries or timeout
    if (!isNaN(handle)) {
      ...
      const elementId = await this.sendPowerShellCommand(
        AutomationElement.rootElement.findFirst(TreeScope.CHILDREN_OR_SELF, condition).buildCommand(),
      );

      if (elementId.trim() !== '') {
        await this.sendPowerShellCommand(
          /* ps1 */ `$rootElement = ${new FoundAutomationElement(elementId).buildCommand()}`,
        );
    ...
    const elementId = await this.sendPowerShellCommand(
      AutomationElement.rootElement.findFirst(TreeScope.CHILDREN, condition).buildCommand(),
    );
    ...
    this.log.info(
      `Failed to locate window with name '${name}'. Sleeping for ${SLEEP_INTERVAL_MS} milliseconds and retrying... (${i}/20)`,
    ); // TODO: make a setting for the number of retries or timeout
...
export async function changeRootElement(
  this: NovaWindowsDriver,
  pathOrNativeWindowHandle: string | number,
): Promise<void> {
  ...
  if (path.includes('!') && path.includes('_') && !(path.includes('/') || path.includes('\\'))) {
    ...
    await this.sendPowerShellCommand(
      /* ps1 */ `Start-Process 'explorer.exe' 'shell:AppsFolder\\${path}'${this.caps.appArguments ? ` -ArgumentList '${this.caps.appArguments}'` : ''}`,
    );
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      this.log.info(
        `Failed to locate window of the app. Sleeping for ${SLEEP_INTERVAL_MS} milliseconds and retrying... (${i}/20)`,
      ); // TODO: make a setting for the number of retries or timeout
      await sleep(SLEEP_INTERVAL_MS); // TODO: make a setting for the sleep timeout
    }
  } else {
    ...
    await this.sendPowerShellCommand(
      /* ps1 */ `Start-Process '${normalizedPath}'${this.caps.appArguments ? ` -ArgumentList '${this.caps.appArguments}'` : ''}`,
    );
    ...
    for (let i = 1; i <= 20; i++) {
      try {
        ...
        const result = await this.sendPowerShellCommand(
          /* ps1 */ `(Get-Process -Name '${processName}' | Sort-Object StartTime -Descending).Id`,
        );
        const processIds = result
          .split('\n')
          .map((pid) => pid.trim())
...
export async function waitForNewWindow(this: NovaWindowsDriver, pid: number, timeout: number): Promise<number> {
  ...
  while (Date.now() - start < timeout) {
    const elements = await this.sendPowerShellCommand(
      AutomationElement.rootElement
        .findAll(TreeScope.CHILDREN, new PropertyCondition(Property.PROCESS_ID, new PSInt32(pid)))
        .buildCommand(),
    );
    ...
    if (elementIds.length > 0) {
      const nativeWindowHandle = await this.sendPowerShellCommand(
        new FoundAutomationElement(elementIds[0]).buildGetPropertyCommand(Property.NATIVE_WINDOW_HANDLE),
      );
      return Number(nativeWindowHandle);
    }

    this.log.debug(
      `Waiting for the process window to appear... (${++attempts}/${Math.floor(timeout / SLEEP_INTERVAL_MS)})`,
    );
...
export async function attachToApplicationWindow(this: NovaWindowsDriver, processIds: number[]): Promise<void> {
  const nativeWindowHandle = await waitForNewWindow.call(
    this,
    processIds[0],
    this.caps['ms:waitForAppLaunch'] ?? SLEEP_INTERVAL_MS * 20,
  );

  let elementId = '';
  for (let i = 1; i <= 20; i++) {
    elementId = await this.sendPowerShellCommand(
      AutomationElement.rootElement
        .findFirst(
          TreeScope.CHILDREN,
          new PropertyCondition(Property.NATIVE_WINDOW_HANDLE, new PSInt32(nativeWindowHandle)),
        )
        .buildCommand(),
    );
    if (elementId) {
    ...
    this.log.info(
      `The window with handle 0x${nativeWindowHandle.toString(16).padStart(8, '0')} is not yet available in the UI Automation tree. Sleeping for ${SLEEP_INTERVAL_MS} milliseconds and retrying... (${i}/20)`,
    ); // TODO: make a setting for the number of retries or timeout
    await sleep(SLEEP_INTERVAL_MS); // TODO: make a setting for the sleep timeout
  }
  ...
  if ((await this.sendPowerShellCommand(/* ps1 */ `$null -ne $rootElement`)).toLowerCase() === 'true') {
    const nativeWindowHandle = Number(
      await this.sendPowerShellCommand(
        AutomationElement.automationRoot.buildGetPropertyCommand(Property.NATIVE_WINDOW_HANDLE),
      ),
    );
