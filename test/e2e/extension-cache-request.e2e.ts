...
describe('windows: cacheRequest', () => {
  it('pushes a cacheRequest with treeScope: SubTree without error', async () => {
    ...
    try {
      await expect(driver.executeScript('windows: cacheRequest', [{treeScope: 'SubTree'}])).resolves.not.toThrow();
    } finally {
  ...
  it('pushes a cacheRequest with treeFilter: RawView without error', async () => {
    const driver = await createCalculatorSession();
    try {
      await expect(driver.executeScript('windows: cacheRequest', [{treeFilter: 'RawView'}])).resolves.not.toThrow();
    } finally {
      await quitSession(driver);
    }
  });
