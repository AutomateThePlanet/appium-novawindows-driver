...
describe('AutomationElement.findFirst', () => {
  ...
  it('uses FIND_FIRST_ANCESTOR_OR_SELF for ANCESTORS_OR_SELF scope', () => {
    const cmd = decodeCommand(
      AutomationElement.automationRoot.findFirst(TreeScope.ANCESTORS_OR_SELF, trueCondition).toString(),
    );
  ...
  it('uses FIND_DESCENDANTS for DESCENDANTS scope', () => {
    const cmd = decodeCommand(
      AutomationElement.automationRoot.findFirst(TreeScope.DESCENDANTS, trueCondition).toString(),
    );
  ...
  it('uses FIND_FOLLOWING for FOLLOWING scope', () => {
    const cmd = decodeCommand(
      AutomationElement.automationRoot.findFirst(TreeScope.FOLLOWING, trueCondition).toString(),
    );
  ...
  it('uses FIND_PRECEDING for PRECEDING scope', () => {
    const cmd = decodeCommand(
      AutomationElement.automationRoot.findFirst(TreeScope.PRECEDING, trueCondition).toString(),
    );
  ...
  it('uses FIND_FOLLOWING_SIBLING for FOLLOWING_SIBLING scope', () => {
    const cmd = decodeCommand(
      AutomationElement.automationRoot.findFirst(TreeScope.FOLLOWING_SIBLING, trueCondition).toString(),
    );
    expect(cmd).toContain('GetNextSibling');
  });
  ...
  it('uses FIND_PRECEDING_SIBLING for PRECEDING_SIBLING scope', () => {
    const cmd = decodeCommand(
      AutomationElement.automationRoot.findFirst(TreeScope.PRECEDING_SIBLING, trueCondition).toString(),
    );
    expect(cmd).toContain('GetPreviousSibling');
  });
  ...
  it('uses FIND_CHILDREN_OR_SELF for CHILDREN_OR_SELF scope', () => {
    const cmd = decodeCommand(
      AutomationElement.automationRoot.findFirst(TreeScope.CHILDREN_OR_SELF, trueCondition).toString(),
    );
    expect(cmd).toContain('FindFirst');
  });
});
...
describe('AutomationElement.findAll', () => {
  ...
  it('uses FIND_ALL_DESCENDANTS for DESCENDANTS scope', () => {
    const cmd = decodeCommand(
      AutomationElement.automationRoot.findAll(TreeScope.DESCENDANTS, trueCondition).toString(),
    );
  ...
  it('uses FIND_ALL_ANCESTOR_OR_SELF for ANCESTORS_OR_SELF scope', () => {
    const cmd = decodeCommand(
      AutomationElement.automationRoot.findAll(TreeScope.ANCESTORS_OR_SELF, trueCondition).toString(),
    );
    expect(cmd).toContain('GetParent');
  });
