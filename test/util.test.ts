...
describe('assertSupportedEasingFunction', () => {
  it.each(['linear', 'ease', 'ease-in', 'ease-out', 'ease-in-out'])('accepts "%s"', (value) => {
  ...
  it('throws for malformed cubic-bezier', () => {
    ...
    expect(() => assertSupportedEasingFunction('cubic-bezier(0.25, 0.1, 0.25)')).toThrow(
      'Unsupported or invalid easing function',
    );
    // Negative x1 (regex only allows non-negative for x1 and x2)
    expect(() => assertSupportedEasingFunction('cubic-bezier(-1, 0, 0, 1)')).toThrow(
      'Unsupported or invalid easing function',
    );
    // Non-numeric
