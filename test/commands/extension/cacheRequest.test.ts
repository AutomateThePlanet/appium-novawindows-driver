...
describe('pushCacheRequest', () => {
  ...
  it('throws when all properties are undefined', async () => {
    ...
    await expect(pushCacheRequest.call(driver, {})).rejects.toThrow(
  ...
  it('sends treeFilter command when treeFilter is set', async () => {
    ...
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining('TreeFilter'));
  });

  it('throws for invalid treeScope value', async () => {
    const driver = createMockDriver() as any;
    await expect(pushCacheRequest.call(driver, {treeScope: 'InvalidScope'})).rejects.toThrow('Invalid value');
  });

  it('throws for invalid automationElementMode value', async () => {
    const driver = createMockDriver() as any;
    await expect(pushCacheRequest.call(driver, {automationElementMode: 'InvalidMode'})).rejects.toThrow(
      'Invalid value',
    );
  });
});
