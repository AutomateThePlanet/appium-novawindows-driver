...
describe('executeClickAndDrag', () => {
  ...
  it('throws when only startX is provided without startY', async () => {
    ...
    (driver as any).caps = {};
    await expect(executeClickAndDrag.call(driver, {startX: 100, endX: 200, endY: 200})).rejects.toThrow(
  ...
  it('throws when only endX is provided without endY', async () => {
    const driver = createMockDriver() as any;
    (driver as any).caps = {};
    await expect(executeClickAndDrag.call(driver, {startX: 0, startY: 0, endX: 100})).rejects.toThrow(
  ...
  it('throws when neither start coords nor startElementId provided', async () => {
    const driver = createMockDriver() as any;
    (driver as any).caps = {};
    await expect(executeClickAndDrag.call(driver, {endX: 100, endY: 100})).rejects.toThrow(
  ...
  it('drags with elementId when element exists', async () => {
    ...
    let callIndex = 0;
    driver.sendPowerShellCommand.mockImplementation(() => Promise.resolve(returnValues[callIndex++] ?? rectJson));
