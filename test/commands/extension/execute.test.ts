...
describe('execute (command router)', () => {
  ...
  it('routes windows:deleteFile to deleteFile with args', async () => {
    ...
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining('Remove-Item'));
  });
  ...
  it('routes windows:invoke to patternInvoke with element', async () => {
    ...
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining('W0ludm9rZVBhdHRlcm5d'));
  });

  it('throws UnknownCommandError for unknown windows command', async () => {
    await expect(extension.execute.call(driver, 'windows: unknownCommand', [])).rejects.toThrow('Unknown command');
  });
  ...
  it('routes powerShell to executePowerShellScript', async () => {
    ...
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining('R2V0LVByb2Nlc3M'));
  });
  ...
  it('routes return window.name to sendPowerShellCommand', async () => {
    ...
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining('JHJvb3RFbGVtZW50'));
    expect(result).toBe('WindowName');
  });

  it('throws NotImplementedError for non-matching script', async () => {
    await expect(extension.execute.call(driver, 'unknownScript', [])).rejects.toThrow('Method is not implemented');
  });
});
