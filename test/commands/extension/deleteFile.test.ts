...
describe('deleteFile', () => {
  ...
  it('sends Remove-Item with -Path for simple paths', async () => {
    ...
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining("-Path 'C:\\temp\\file.txt'"));
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining('-Force'));
  });

  it('uses -LiteralPath when path contains brackets', async () => {
    const driver = createMockDriver() as any;
    await deleteFile.call(driver, {path: 'C:\\temp\\file[1].txt'});
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining('-LiteralPath'));
  });

  it('escapes single quotes in path', async () => {
    const driver = createMockDriver() as any;
    await deleteFile.call(driver, {path: "C:\\temp\\file's.txt"});
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining("file''s.txt"));
  });
});
