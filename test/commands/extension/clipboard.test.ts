...
describe('getClipboardBase64', () => {
  ...
  it('returns plaintext clipboard by default', async () => {
    ...
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining('Get-Clipboard'));
...
describe('setClipboardFromBase64', () => {
  ...
  it('sets plaintext clipboard by default', async () => {
    ...
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining('FromBase64String'));
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining('Set-Clipboard'));
  });
  ...
  it('sets plaintext clipboard with explicit contentType', async () => {
    ...
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining('FromBase64String'));
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining('dGVzdA=='));
  });
  ...
  it('sets image clipboard', async () => {
    ...
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining('FromBase64String'));
    expect(driver.sendPowerShellCommand).toHaveBeenCalledWith(expect.stringContaining('SetImage'));
  });
